...
import { Config, UnsavedGame } from '../models'; // Importing mongoose models
import { z } from 'zod';
import { disconnectClient, emit, spectate } from '@arken/node/util/player';
import { testMode, baseConfig, sharedConfig } from './app.config';
...
async function normalizeAddress(app, address) {
...
async function isValidSignatureRequest(app, req) {
  log('Verifying', req);
...
function emitDirect(app, socket, eventName, eventData) {
...
function emit(app, client, ...args) {
  ...
  emitDirect(app, socket, 'Events', getPayload(compiled));
}

async function spawnRandomReward(app) {
...
function disconnectAllPlayers(app) {
  ...
  for (let i = 0; i < app.clients.length; i++) {
    ...
    disconnectClient(app, client, 'disconnect all players');
  }
}
...
function isMechanicEnabled(app, player, mechanicId) {
...
async function claimReward(app, player, reward) {
  if (!reward) return;
  ...
  if (isMechanicEnabled(app, player, 1164) && player.character.meta[1164] > 0) {
...
function randomizeSpriteXp(app) {
...
function syncSprites(app) {
  ...
  if (app.powerups.length > length) {
    ...
    spawnSprites(app, length - app.powerups.length);
  }
}

export function isRateLimited(app, address) {
  const now = Date.now();
...
function disconnectClient(app, player, reason = 'Unknown', immediate = false) {
...
function randomRoundPreset(app) {
...
function spawnSprites(app, amount) {
  for (let i = 0; i < amount; i++) {
    const position = getUnobstructedPosition(app);
...
function addToRecentPlayers(app, player) {
...
async function isValidAdminRequest(app, req) {
  log('Verifying Admin', req);
  if (!req.signature?.address) return false;
  if (req.signature.address.length !== 42 || req.signature.address.slice(0, 2) !== '0x') return false;

  if (isRateLimited(app, req.signature.address)) {
...
function roundEndingSoon(app, sec) {
...
const registerKill = (app, winner, loser) => {
  ...
  if (app.config.gameMode !== 'Pandamonium' || !app.pandas.includes(winner.address)) {
    ...
    const notReallyTrying = app.config.antifeed1
      ? (totalKills >= 2 && loser.kills < 2 && loser.rewards <= 1) ||
        (totalKills >= 2 && loser.kills < 2 && loser.app.powerups <= 100)
      : false;
  ...
  if (isMechanicEnabled(app, loser, 1102) && loser.character.meta[1102] > 0) {
    const r = random(1, 100);
  ...
  if (isMechanicEnabled(app, winner, 1222) && winner.character.meta[1222] > 0) {
  ...
  if (isMechanicEnabled(app, winner, 1219) && winner.character.meta[1219] > 0) {
  ...
  disconnectClient(app, loser, 'got killed');
  const orb = {
    id: shortId.generate(),
    type: 4,
    points: orbPoints,
  ...
  if (app.config.orbOnDeathPercent > 0 && !roundEndingSoon(app, app.config.orbCutoffSeconds)) {
    setTimeout(() => {
...
function sendUpdates(app) {
  ...
  flushEventQueue(app);
  setTimeout(() => sendUpdates(app), app.config.sendUpdateLoopSeconds * 1000);
}

function spawnRewards(app) {
  spawnRandomReward(app);
  setTimeout(() => spawnRewards(app), app.config.rewardSpawnLoopSeconds * 1000);
}

function getRoundInfo(app) {
  return Object.keys(sharedConfig)
    .sort()
    .reduce((obj, key) => {
      obj.push(app.config[key]);
      return obj;
    }, []);
}
...
async function calcRoundRewards(app) {
  ...
  if (calcRewardsRes?.data) {
    ...
    if (app.config.rewardWinnerAmount === 0 && calcRewardsRes.data.rewardWinnerAmount !== 0) {
      ...
      emitAll(
        'onSetRoundInfo',
        roundTimer + ':' + getRoundInfo(app).join(':') + ':' + getGameModeGuide(app, app.config).join(':')
      );
    }
  }
}
...
async function resetLeaderboard(app, preset = null) {
  try {
    ...
    if (app.config.calcRoundRewards) {
      await calcRoundRewards(app);
    }
    if (preset) {
      app.roundConfig = { ...baseConfig, ...sharedConfig, ...preset };
      app.config = JSON.parse(JSON.stringify(app.roundConfig));
    } else {
      randomRoundPreset(app);
    }
    ...
    for (const client of app.clients) {
      ...
      client.speed = getClientSpeed(app, client, app.config);
    ...
    log(
      ...
      (
        ...
        ':' +
        getRoundInfo(app).join(':') +
        ':' +
        getGameModeGuide(app, app.config).join(':')
      ).split(':').length
    );
    emitAll('onClearLeaderboard');
  ...
  if (!app.config.noBoot && !app.config.isRoundPaused) {
    ...
    for (const client of app.clients) {
      ...
      if (client.lastReportedTime <= oneMinuteAgo) {
        ...
        disconnectClient(app, client, 'timed out');
      }
    }
  }
  setTimeout(() => checkConnectionLoop(app), app.config.checkConnectionLoopSeconds * 1000);
}
...
function slowGameloop(app) {
  ...
  setTimeout(() => slowGameloop(app), app.config.slowLoopSeconds * 1000);
}

function resetPlayer(app, player) {
...
async function detectCollisions(app) {
  try {
    const now = getTime();
    ...
    for (let i = 0; i < app.clients.length; i++) {
      ...
      if (!Number.isFinite(player.position.x) || !Number.isFinite(player.speed)) {
        ...
        disconnectClient(app, player, 'speed problem');
        continue;
      }
    ...
    if (app.config.level2allowed) {
      if (
        ...
        if (!app.config.level2open) {
          ...
          clearSprites(app);
          spawnSprites(app, app.config.spritesStartCount);
    ...
    if (!app.config.isRoundPaused) {
      ...
      for (let i = 0; i < app.clients.length; i++) {
        ...
        for (const powerup of app.powerups) {
          ...
          if (isMechanicEnabled(app, player, 1117) && player.character.meta[1117] > 0) {
            player.xp +=
...
function fastGameloop(app) {
  try {
    ...
    for (let i = 0; i < app.clients.length; i++) {
      ...
      if (!app.config.isRoundPaused && app.config.gameMode !== 'Pandamonium') {
        ...
        if (isMechanicEnabled(app, client, 1105) && isMechanicEnabled(app, client, 1104)) {
        ...
        if (client.xp > client.maxHp) {
          if (decay > 0) {
            if (client.avatar < app.config.maxEvolves - 1) {
              ...
              if (isMechanicEnabled(app, client, 1223) && client.character.meta[1223] > 0) {
                client.overrideSpeedUntil = getTime() + 1000;
                client.overrideSpeed = client.speed * (1 + client.character.meta[1223] / 100);
                if (isMechanicEnabled(app, client, 1030) && client.character.meta[1030] > 0) {
              ...
              if (client.avatar === 0) {
                ...
                if (!app.config.noBoot && !isInvincible && !isNew && !app.config.isGodParty) {
                  ...
                  if (client.lastTouchTime > now - 2000) {
                    registerKill(app, app.clientLookup[client.lastTouchPlayerId], client);
                  } else {
                    disconnectClient(app, client, 'starved');
                  }
                } else {
                  client.xp = client.maxHp;
                  client.avatar = Math.max(
  ...
  setTimeout(() => fastGameloop(app), app.config.fastLoopSeconds * 1000);
}

function getGameModeGuide(app, config) {
  return (
    app.config.guide || [
...
function flushEventQueue(app) {
  ...
  if (app.eventQueue.length) {
    ...
    emitAllDirect(app, 'Events', getPayload(compiled));
    app.eventQueue = null;
    app.eventQueue = [];
  }
}

function broadcastMechanics(app, client) {
  if (isMechanicEnabled(app, client, 1150))
    emit(
      client,
      'onBroadcast',
  ...
  if (isMechanicEnabled(app, client, 1117))
    emit(
      client,
      'onBroadcast',
...
// Middleware to validate admin request
const validateMod = t.middleware(async ({ input, ctx, next }) => {
  const isValid = await isValidAdminRequest(app, input);
  if (!isValid) {
    return { status: 0 };
  }
  return next();
...
export const appRouter = t.router({
  connected: t.procedure
    ...
    .input(z.object({ id: z.string(), signature: z.any() }))
    .mutation(async ({ input, ctx: { app, socket, client } }) => {
      ...
      for (const observer of sameNetworkObservers) {
        disconnectClient(app, observer, 'same network observer');
      }
  ...
  apiConnected: t.procedure
    ...
    .use(customErrorFormatter)
    .input(z.object({ id: z.string(), signature: z.any() }))
    .mutation(async ({ input, ctx: { app, client } }) => {
  ...
  load: t.procedure
    .use(customErrorFormatter)
    .input(z.object({}))
    .mutation(async ({ ctx: { app, socket, client } }) => {
      log('Load', client.hash);
      emit(client, 'OnLoaded', 1);
      return { status: 1 };
    }),

  spectate: t.procedure
    .use(customErrorFormatter)
    .input(z.object({}))
    .mutation(async ({ ctx: { app, client } }) => {
    ...
    .mutation(async ({ input, ctx: { app, socket, client } }) => {
      ...
      try {
        ...
        if (!pack.signature || !pack.network || !pack.device || !pack.address) {
          ...
          disconnectClient(app, client, 'signin problem');
          return { status: 0 };
        }

        const address = await normalizeAddress(app, pack.address);
        ...
        if (
          ...
          disconnectClient(app, client, 'signature problem');
          return { status: 0 };
        }

        if (client.isBanned) {
          emit(client, 'OnBanned', true);
          disconnectClient(app, client, 'banned');
          return { status: 0 };
        }
        ...
        if (app.config.isMaintenance && !client.isMod) {
          ...
          disconnectClient(app, client, 'maintenance');
          return { status: 0 };
        }

        let name = app.addressToUsername[address] || (await getUsername(address)) || generateGuestName(app);
        ...
        if (client.name !== name || client.address !== address) {
          ...
          if (recentPlayer && now - recentPlayer.lastUpdate < 3000) {
            ...
            disconnectClient(app, client, 'joined too soon', true);
            return { status: 0 };
          }
    ...
    .mutation(async ({ ctx: { app, client } }) => {
      ...
      try {
        ...
        if (confirmUser?.status !== 1) {
          ...
          disconnectClient(app, client, 'failed realm check');
          return { status: 0 };
        }
        ...
        if (recentPlayer && now - recentPlayer.lastUpdate < 3000) {
          ...
          disconnectClient(app, client, 'connected too soon');
          return { status: 0 };
        }
        ...
        if (app.observers.length === 0) {
          ...
          disconnectClient(app, client, 'realm not connected');
          return { status: 0 };
        }
        ...
        log('Error:', e);
        disconnectClient(app, client, 'not sure: ' + e);
        return { status: 0 };
      }
    }),
    ...
    .mutation(async ({ input, ctx: { app, socket, client } }) => {
      ...
      try {
        ...
        if (
          ...
          disconnectClient(app, client, 'position jumped');
          return { status: 0 };
        }
  ...
  startRound: t.procedure
    .use(validateMod)
    .use(customErrorFormatter)
    .input(z.object({ id: z.string(), signature: z.any(), data: z.any() }))
    .mutation(async ({ input, ctx: { app, client } }) => {
      clearTimeout(app.roundLoopTimeout);
      if (app.config.isRoundPaused) {
  ...
  messageUser: t.procedure
    .use(validateMod)
    .use(customErrorFormatter)
    .input(z.object({ id: z.string(), data: z.any(), signature: z.any() }))
    .mutation(async ({ input, ctx: { app, client } }) => {
  ...
  changeUser: t.procedure
    ...
    .mutation(async ({ input, ctx: { app, client } }) => {
      ...
      for (const key of Object.keys(input.data.app.config)) {
        ...
        if (client.hasOwnProperty(key)) newClient[key] = val;
  ...
  kickUser: t.procedure
    ...
    .mutation(async ({ input, ctx: { app, client } }) => {
      const targetClient = app.clients.find((c) => c.address === input.data.target);
      if (!targetClient) return { status: 0 };
      disconnectClient(app, targetClient, 'kicked');
      return { status: 1 };
    }),
...
async function initEventHandler(app) {
    ...
    try {
      ...
      if (!testMode && app.killSameNetworkClients) {
        ...
        if (sameNetworkClient) {
          ...
          disconnectClient(app, client, 'same network');
          return;
        }
      }
      ...
      socket.on('disconnect', function () {
        ...
        disconnectClient(app, client, 'client disconnected');
...
export async function initGameServer(app) {
  try {
    ...
    app.rateLimitWindow = 60 * 1000; // 1 minute
