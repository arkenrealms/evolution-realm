...
import { TRPCLink, Operation } from '@trpc/client';
...
import * as roomClientSchema from '../room-client';
import type * as RoomClientSchema from '../room-client';
import type {
  Orb,
  RoomApplication,
  RoomClient,
...
import { Mechanic, createRoomClientRouter } from '../types';
...
class Room {
  ...
  clientLookup: Record<string, RoomClient>;
  powerups: PowerUp[];
  ...
  app: RoomApplication;
  emit: ReturnType<typeof createTRPCProxyClient<RoomClientRouter>>;
  emitDirect: ReturnType<typeof createTRPCProxyClient<RoomClientRouter>>;
  emitAll: ReturnType<typeof createTRPCProxyClient<RoomClientRouter>>;
  emitAllDirect: ReturnType<typeof createTRPCProxyClient<RoomClientRouter>>;

  constructor(app: RoomApplication) {
    this.app = app;
    ...
    this.emit = createTRPCProxyClient<RoomClientRouter>({
      links: [
        () =>
          ({ op, next }) => {
            return observable((observer) => {
              ...
              (context.client as RoomClient).socket.emit('onEvents', this.getPayload(compiled));
    ...
    this.emitDirect = createTRPCProxyClient<RoomClientRouter>({
      links: [
        () =>
          ({ op, next }) => {
            return observable((observer) => {
              ...
              (context.client as RoomClient).socket.emit(name, Object.values(args));
    ...
    this.emitAll = createTRPCProxyClient<RoomClientRouter>({
      links: [
        () =>
          ({ op, next }) => {
            return observable((observer) => {
    ...
    this.emitAllDirect = createTRPCProxyClient<RoomClientRouter>({
      links: [
        () =>
          ({ op, next }) => {
            return observable((observer) => {
  ...
  handleClientDecay(client: RoomClient, decay: number, now: number, isInvincible: boolean, currentTime: number): void {
  ...
  registerKill(winner: RoomClient, loser: RoomClient): void {
    const now = this.getTime();
  ...
  async connected(input: Schema.ConnectedInput, { client }: RoomContext) {
    if (this.realm.client?.socket?.connected) {
  ...
  addToRecentClients(client: RoomClient): void {
  ...
  async apiConnected(input: Schema.ApiConnectedInput, ctx: RoomContext) {
    this.emitAll.onBroadcast.mutate({ message: 'API connected', priority: 0 });
    return { status: 1 };
  }

  async apiDisconnected(input: Schema.ApiDisconnectedInput, ctx: RoomContext) {
    this.emitAll.onBroadcast.mutate({ message: 'API disconnected', priority: 0 });
    return { status: 1 };
  }

  // Example usage in your function:
  broadcastMechanics(input: Schema.BroadcastInput, { client }: RoomContext): void {
  ...
  isMechanicEnabled({ id }: { id: number }, { client }: RoomContext): boolean {
    return !!client.character.meta[id];
  }

  async setCharacter(input: Schema.SetCharacterInput, { client }: RoomContext) {
  ...
  async setConfig(input: Schema.SetConfigInput, { client }: RoomContext) {
    return { status: 1 };
  }

  async getConfig(input: Schema.GetConfigInput, { client }: RoomContext) {
    return { status: 1, data: this.config };
  }

  async load(input: Schema.LoadInput, { client }: RoomContext) {
    log('Load', client.hash);
    this.emit.onLoaded.mutate(1, { client });
    return { status: 1 };
  }

  public spectate(input: Schema.SpectateInput, { client }: RoomContext) {
  ...
  disconnectClient(client: RoomClient, reason = 'Unknown', immediate = false) {
  ...
  async setInfo({ msg }: Schema.SetInfoInput, { client }: RoomContext) {
    log('SetInfo', msg);
  ...
  compareClients(a: RoomClient, b: RoomClient): number {
  ...
  async auth({ data, signature }: Schema.AuthInput, { client }: RoomContext) {
    log('Verifying', data);
  ...
  getClientSpeed(client: RoomClient): number {
  ...
  public async claimReward(client: RoomClient, reward: Reward): Promise<void> {
    if (!reward) return;
  ...
  async updateMyself({ msg }: Schema.UpdateMyselfInput, { client }: RoomContext) {
  ...
  async restart(input: Schema.RestartInput, ctx: RoomContext) {
  ...
  async maintenance(input: Schema.MaintenanceInput, ctx: RoomContext) {
  ...
  async unmaintenance(input: Schema.UnmaintenanceInput, ctx: RoomContext) {
  ...
  async startBattleRoyale(input: Schema.StartBattleRoyaleInput, ctx: RoomContext) {
  ...
  async stopBattleRoyale(input: Schema.StopBattleRoyaleInput, ctx: RoomContext) {
    this.baseConfig.isBattleRoyale = false;
    this.config.isBattleRoyale = false;
  ...
  async pauseRound(input: Schema.PauseRoundInput, ctx: RoomContext) {
  ...
  async startRound(input: Schema.StartRoundInput, ctx: RoomContext) {
    clearTimeout(this.roundLoopTimeout);
    if (this.config.isRoundPaused) {
  ...
  async enableForceLevel2(input: Schema.EnableForceLevel2Input, ctx: RoomContext) {
    this.baseConfig.level2forced = true;
    this.config.level2forced = true;
    return { status: 1 };
  }

  async disableForceLevel2(input: Schema.DisableForceLevel2Input, ctx: RoomContext) {
    this.baseConfig.level2forced = false;
    this.config.level2forced = false;
    return { status: 1 };
  }

  async startGodParty(input: Schema.StartGodPartyInput, ctx: RoomContext) {
    this.baseConfig.isGodParty = true;
    this.config.isGodParty = true;
  ...
  async stopGodParty(input: Schema.StopGodPartyInput, ctx: RoomContext) {
    this.baseConfig.isGodParty = false;
    this.config.isGodParty = false;
    for (const client of this.clients) {
  ...
  async startRoyale(input: Schema.StartBattleRoyaleInput, ctx: RoomContext) {
  ...
  async pauseRoyale(input: Schema.PauseRoundInput, ctx: RoomContext) {
    this.emitAll.onBroadcast.mutate({ message: `Royale Paused`, priority: 2 });
    return { status: 1 };
  }

  async unpauseRoyale(input: Schema.UnpauseRoyaleInput, ctx: RoomContext) {
    this.emitAll.onBroadcast.mutate({ message: `Royale Unpaused`, priority: 2 });
    return { status: 1 };
  }

  async stopRoyale(input: Schema.StopBattleRoyaleInput, ctx: RoomContext) {
  ...
  async makeBattleHarder(input: Schema.MakeBattleHarderInput, ctx: RoomContext) {
  ...
  async makeBattleEasier(ctx: { client: RoomClient }, input: { signature: string }) {
    this.baseConfig.dynamicDecayPower = false;
    this.config.dynamicDecayPower = false;
    this.sharedConfig.decayPower -= 2;
    this.config.decayPower -= 2;
  ...
  async resetBattleDifficulty(ctx: { client: RoomClient }, input: { signature: string }) {
  ...
  async messageUser(ctx: { client: RoomClient }, input: { data: any; signature: string }) {
  ...
  async changeUser(ctx: { client: RoomClient }, input: { data: any; signature: string }) {
  ...
  async broadcast(ctx: { client: RoomClient }, input: { data: any; signature: string }) {
    this.emitAll.onBroadcast.mutate(input.data.message.replace(/:/gi, ''), 0);
    return { status: 1 };
  }

  async kickClient(ctx: { client: RoomClient }, input: { data: any; signature: string }) {
    const targetClient = this.clients.find((c) => c.address === input.data.target);
    if (!targetClient) return { status: 0 };
  ...
  async info(ctx: { client: RoomClient }, input: any) {
    return {
      status: 1,
      data: {
        id: this.config.id,
...
export const createRoomRouter = (gameWorld: Room) => {
  return t.router({
    connected: t.procedure
      .use(hasRole('realm', t))
      .use(customErrorFormatter(t))
...
export type Router = typeof createRoomRouter;

export async function init(app) {
  try {
    const gameWorld = new Room(this.app);
    ...
    this.app.io.on('connection', function (socket) {
      try {
        ...
        const router = createRoomRouter(gameWorld);
